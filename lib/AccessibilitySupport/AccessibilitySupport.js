...
var defaultObservers = [
	{from: 'accessibilityDisabled', to: 'aria-hidden'},
...
function registerAriaUpdate (obj) {
	if (!obj.pending) {
		obj.pending = true;
		this.startJob('updateAriaAttributes', this.bindSafely(updateAriaAttributes), 16);
	}
}
...
function initAriaObserver (control, c) {
	var
		...
		// method is either 'method', if it exists, or if a 'to' path exists, this is a
		// binding-style config so we'll use the internal toAriaAttribute method
		method = c.method && control.bindSafely(c.method) ||
				c.to && control.bindSafely(toAriaAttribute, c.path || c.from, c.to) ||
				null,
...
module.exports = {
	...
	*/
	setAriaAttribute: function (name, value) {
		value = !this.accessibilityDisabled || name == 'aria-hidden' ? value : null;
