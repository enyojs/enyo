...
var AnimationSupport = {
	...
	* @public
	*/
	getInitial: function (accelerate) {
		var dom = this.hasNode(),
			prop = this.getAnimation(),
			init = frame.getCompoutedProperty(dom, prop, this._start);

		utils.mixin(this, init);
		if (accelerate) frame.accelerate(dom, this.matrix);
		
	},

	/**
	* Gets HTML representation of this character
	* @public
	*/
	getDom: function() {
