...
module.exports = {
	...
	*/
	...
	updateDelta: function (charc, d) {
		...
		var k, cV, eV, mod, end = {}, dist, rev;
		for (k in charc._prop) {
			cV = (charc.currentState || charc._startAnim)[k];
			//cV = k == 'rotate' ? Vector.toVector(cV) : cV;
			eV = utils.isString(charc._prop[k]) ? frame.parseValue(charc._prop[k]) : charc._prop[k];
			eV = k == 'rotate' ? Vector.toQuant(eV) : eV;

			// eV = k === 'rotate' ? Vector.quantCross(eV, cV) : Vector.add(eV, cV);
			// rev = Vector.direction(eV, cV);
			// if(rev) cV =  Vector.quantInverse(cV);
			dist = (k == 'rotate' ? Vector.quantDistance : Vector.distance)(eV, cV);
			//dV = Vector.subtract(frame.copy(cV), eV);
			console.log("distance: " + k, dist);
			console.log("direction: " + k, rev);
			...
			if (dist === 0) {
				...
				if(rev) {
					 ...
					 end[k] = k === 'rotate' ? Vector.quantCross(cV, mod) : Vector.add(cV, mod);
					 end[k] = Vector.quantInverse(end[k]);
				} else {
					end[k] = k === 'rotate' ? Vector.quantCross(mod, cV) : Vector.add(cV, mod);	
				}
				//cV = k == 'rotate' ? Vector.toQuant(cV) : cV;
				// if (!rev) {
					// end[k] = k === 'rotate' ? Vector.quantCross(mod, cV) : Vector.add(cV, mod);	
					console.log("direction: " + k, end[k]);
				// } else 
				// 	end[k] = k === 'rotate' ? Vector.quantCross(mod, Vector.quantInverse(cV)) : Vector.subtract(cV, mod);
				//end[k] = k === 'rotate' ? Vector.quantCross(mod, cV) : Vector.add(cV, mod);
			}
		}
