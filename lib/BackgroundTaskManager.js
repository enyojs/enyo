...
module.exports = kind.singleton(
	...
	*
	* @param {Object} sender - The customer which has had a change in priority for one of its
	*	tasks.
	* @param {String} ev - The name of the event.
	* @param {Number} priority - The priority that will be checked for urgency.
	* @private
	*/
	notifyPriority: function (sender, ev, priority) {
		var idx;

		if (priority == Priorities.SOON) {
			idx = this.customers.indexOf(sender);

			if (idx > -1) {
				this.customers.slice(idx, 1);
				this.customers.unshift(sender);
			}
		}
