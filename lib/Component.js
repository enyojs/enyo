...
/**
* Contains the declaration for the {@link enyo.Component} kind.
...
* @definedby module:enyo/Component
* @public
*/
var Component = module.exports = kind(
	...
	*/
	published:
		...
		*/
		...
		/**
		* The default set of keys, corresponding to child component kind definitions, which are
		* effectively "ignored" when determining whether or not the control's child components,
		* overall, have changed enough to warrant a complete re-render. For each of these
		* stateful keys, changes will be tracked on a per-key basis via hashing, such that it
		* can be determined whenever state changes and the corresponding component's state can
		* be specifically updated. When {@link enyo.Component#updateComponents} is invoked, this
		* set of stateful keys is utilized by default, if no stateful keys are provided.
		*
		* @type {Array}
	...
	*	this component.
	* @param {Array} [keys] - An optional set of keys to ignore when determining whether
	*	or not the specified kind definitions differ from the existing definitions. If this is
	*	not specified, the default [stateful keys]{@link enyo.Component#statefulKeys} will be
	*	used.
	* @public
	*/
	updateComponents: function (comps, keys) {
		keys = keys || this.statefulKeys;

		var diff = this.computeComponentsDiff(comps, keys),
			comp, controls, control, key, idxKey, idxComp;

		if (diff[this.id] === true) {
			this.destroyClientControls();
			this.createComponents(comps);
		} else {
			for (idxKey = 0; idxKey < keys.length; idxKey++) {
				key = keys[idxKey];
	...
	* @private
	*/
	computeComponentsDiff: function (comps, keys) {
		var hashes = this.computeComponentsHash(comps, keys),
			diff = {},
			key;

		if (this._compHashes) {
			for (key in this._compHashes) {
				diff[key] = this._compHashes[key] != hashes[key];
			}
		} else {
			for (key in hashes) {
				diff[key] = true;
	...
	* @private
	*/
	computeComponentsHash: function (comps, keys) {
		var hashes = {},
			hash, str, filtered, chr, len, idx, key;

		var filterComps = function (comp) {
			var objKeys = Object.keys(comp),
				obj = {},
				idx, key;
		...
		filtered = comps.map(filterComps);

		hashes[this.id] = JSON.stringify(filtered);
