...
var EnyoHistory = module.exports = kind.singleton(
	...
	*
	* @param  {module:enyo/History~HistoryEntry} entry Object describing the history entry. Must
	*	contain a `handler` property to invoke the callback. If it also has a `context` property,
	*	the `handler` will be called with that context. The entry object will be included as the
	...
	*/
	pushEntry: function (entry, silenced) {
		var id;
		_history.push(entry);
		id = entry.context && entry.context.id || 'anonymous';
		if (_supports && !silenced) {
			global.history.pushState({id: id}, '', '');
		}
	},
