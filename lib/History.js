...
var EnyoHistory = module.exports = kind.singleton(
	...
	*/
	...
	/**
	* When true, enyo/History will handle onpopstate events and notify controls when their
	* history entry is popped.
	* When it is false, enyo/History will handle onkeyup event
	*
	* @type {Boolean}
	* @default true
	* @public
	...
	*/
	enqueuePop: function (type, count) {
		...
		if (this.enableHistoryAPI && _queue.length === 1) {
	...
	*/
	pushEntry: function (entry, silenced) {
		...
		_history.push(entry);
		if (this.enableHistoryAPI && _supports && !silenced) {
	...
	*/
	handlePop: function (event) {
		if (this.enableHistoryAPI && _history.length) {
	...
	*/
	handleKeyUp: function (sender, event) {
		if (this.enableHistoryAPI) return;
