...
/**
* Contains the declaration for the {@link enyo.LightPanels} kind.
...
* @definedby module:enyo/LightPanels
* @ui
* @public
*/
module.exports = kind(
	...
	* @private
	*/
	classes: 'enyo-light-panels enyo-light-panels-client',
	...
	*/
	...
	/**
	* We set initial index as 0 when there is static components block.
	* @private
	*/
	determineInitialIndex: function() {
		if (this.index == -1 && this.getPanels().length > 0) {
			this.index = 0;
		}
	},
	...
	* @private
	*/
	findPanelsContainer: function() {
		// Set css property to the direct parent of panels, enyo-light-panels-client,
		// to simplify css rules on ui.css.
		this.panelsContainer = this.controlParent ? this.controlParent : this;
	},
	...
	*/
	orientationChanged: function () {
		...
		this.panelsContainer && this.panelsContainer.addClass(this.orientation);
	},
	...
	*/
	directionChanged: function () {
		...
		this.panelsContainer && this.panelsContainer.addClass(this.direction);
	},
	...
	* @private
	*/
	rememberQueueIndex: function(was) {
		if (this.index - was > 1 || was - this.index > 1) {
			// Cache distance of index
			if (this.queuedIndex === undefined) {
				this.indexDistance = Math.abs(this.index - was);
			}
			// Cache target index
			this.queuedIndex = this.index;
			this.index = was + (this.index - was > 1 ? 1 : -1);
		}
		if (this.queuedIndex == this.index) {
			this.queuedIndex = undefined;
		}
	},
	...
	*/
	pushPanel: function (info, moreInfo, opts) {
		...
		var lastIndex = this.getPanels().length - 1,
			viewId = this.getViewId(info),
			nextPanel = (this.cacheViews && viewId && this.restoreView(viewId)) || this.createComponent(info, moreInfo),
	...
	*/
	pushPanels: function (info, moreInfo, opts) {
		...
		for (idx = 0; idx < info.length; idx++) {
			viewId = this.getViewId(info),
			newPanel = (this.cacheViews && viewId && this.restoreView(viewId)) || this.createComponent(info[idx], moreInfo);
	...
	*/
	setupTransitions: function (previousIndex, animate) {
		...
		if (nextPanel) {
			...
			if (animate) {
				this.initTransition(nextPanel, (this._indexDirection > 0 ? 'forward' : 'backward'));
	...
	* @private
	*/
	initTransition: function(nextPanel, direction) {
		var timing = this.fetchTimingOption(direction);
		this.applyTimingOption(nextPanel, timing);
		if (this._currentPanel) {
			this.applyTimingOption(this._currentPanel, timing);
		}
	},
	...
	*/
	fetchTimingOption: function(direction) {
		var timing = {};
		timing.duration = this.duration / this.indexDistance;
		timing.timingFunction = this.timingFunction;
		timing.delay = this.delay ? this.delay : 0;
		return timing;
	},
	...
	* @private
	*/
	applyTimingOption: function(panel, timing) {
		var trans = 'transform ' + timing.duration + 'ms ' + timing.timingFunction + ' ' + timing.delay + 'ms',
			wTrans = '-webkit-' + trans;
		panel.applyStyle('-webkit-transition', wTrans);
		panel.applyStyle('transition', trans);
		panel.addClass('transitioning');
	},
