...
module.exports = kind(
	...
	* @private
	*/
	handleStateChange: function (panel, was, is) {
		if (was == States.ACTIVATING || was == States.DEACTIVATING) {
			...
			if ((this._currentPanel.state == States.ACTIVE) &&
				(!this._previousPanel || this._previousPanel.state == null))
	...
	*/
	finishTransition: function () {
		if (this._previousPanel) this._previousPanel.unobserve('state', this._prevStateHandler);
	...
	*/
	setupTransitions: function (previousIndex, animate) {
		...
		if (nextPanel) {
			...
			this._nextStateHandler = this.bindSafely(function (was, is) {
				this.handleStateChange(nextPanel, was, is);
			});
			nextPanel.observe('state', this._nextStateHandler);

			if (currPanel) {
				currPanel.set('state', null);
	...
	*/
	applyTransitions: function (nextPanel, direct) {
		...
		if (!this.shouldAnimate() || direct) { // ensure that `transitionFinished is called, regardless of animation
			nextPanel.set('state', States.ACTIVE);
			if (previousPanel) previousPanel.set('state', null);
		}
	},
