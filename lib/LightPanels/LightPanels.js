...
module.exports = kind(
	...
	* @private
	*/
	handleStateChange: function (panel) {
		if (panel.state & States.TRANSITIONED) {
			...
			if ((this._currentPanel.state & States.TRANSITIONED) &&
				(!this._previousPanel || (this._previousPanel.state & States.TRANSITIONED)))
	...
	*/
	finishTransition: function () {
		this._currentPanel.set('state', this._currentPanel.state & ~States.TRANSITIONED);
		if (this._previousPanel) {
			this._previousPanel.set('state', this._previousPanel.state & ~States.TRANSITIONED);
			this._previousPanel.unobserve('state', this._prevStateHandler);
		}
	...
	*/
	setupTransitions: function (previousIndex, animate) {
		...
		if (nextPanel) {
			...
			this._prevStateHandler = this._nextStateHandler;
			this._nextStateHandler = this.bindSafely(function () {
				this.handleStateChange(nextPanel);
			});
			nextPanel.observe('state', this._nextStateHandler);

			if (currPanel) {
				currPanel.set('state', currPanel.state & ~States.ACTIVE);
			...
			nextPanel.set('state', nextPanel.state | States.ACTIVE);
	...
	*/
	applyTransitions: function (nextPanel, direct) {
		...
		nextPanel.set('state', nextPanel.state | States.TRANSITIONING);
		...
		if (this._currentPanel) { // apply the transition for the current panel
			this._currentPanel.set('state', this._currentPanel.state | States.TRANSITIONING);
		...
		if (!this.shouldAnimate() || direct) { // ensure that `transitionFinished is called, regardless of animation
			nextPanel.set('state', nextPanel.state | States.TRANSITIONED);
			if (previousPanel) previousPanel.set('state', previousPanel.state | States.TRANSITIONED);
		}
	},
