...
/**
* The configurable options used by {@link module:enyo/LightPanels~LightPanels} when updating the
* panel index.
*
* @typedef {Object} enyo.LightPanels~UpdateIndexOptions
* @property {Boolean} [direct] - If `true`, the transition to the panel whose index we are
*	changing to will not be animated.
*/
...
module.exports = kind(
	...
	*/
	indexChanged: function (was, is) {
		this.updateIndex(is, {direct: !this.shouldAnimate()}, was);
	},
	...
	*/
	updateIndex: function (to, opts, from) {
		var direct = opts && opts.direct;
		from = from != null ? from : this.index;
		if (this.index != to) this.index = to;
		this.setupTransitions(from, !direct);
	},
