enyo.kind({
	...
	rendered: function() {
		this.inherited(arguments);
		this.sizeable = this.findShowingSibling(this.delta);
		this.moveable = this.findShowingSibling(-this.delta);
	},
	findShowingSibling: function(inDelta) {
		var i = this.parent.indexOfChild(this) - inDelta;
		var c$ = this.parent.children;
		for (var c; (c=c$[i]); i += inDelta) {
			if (c.showing) {
				return c;
			}
		}
	...
	dragHandler: function(inSender, inEvent) {
		...
		this.applyStyle("left", v + "px");
		this.moveable.applyStyle("left", v + this.size + "px");
		this.parent.resized();
		this.doResizing();
		return true;
	},
