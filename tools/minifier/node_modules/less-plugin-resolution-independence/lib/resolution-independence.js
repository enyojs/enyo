	...
	ResolutionIndependence.prototype = {
		...
		*/
		parseValue: function (value, unit) {
			...
			// String value in our to-be-converted unit
				...
				return (scaledValue && scaledValue <= this._minUnitSize) ?
					...
					this.adjustValue(value) / this._baseSize + this._riUnit;
			}
			// LESS object with separate value and unit properties
				...
				if (unit == this._unit) {
					...
					return (scaledValue && scaledValue <= this._minUnitSize) ? 
						...
						{
							value: this.adjustValue(value) / this._baseSize,
							unit: this._riUnit
						};
				}
		...
		adjustValue: function (value) {
			if ((value + 1) % 3 === 0) return value + 1;
			else if ((value - 1) % 3 === 0) return value - 1;
